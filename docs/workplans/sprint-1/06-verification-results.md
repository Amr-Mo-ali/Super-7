...
## Lifecycle graceful-shutdown hotfix (2026-08-31)

**Implemented and production-wired.** The approved internal setting `analysis_shutdown_grace_seconds` defaults to **5.0 seconds**, is parsed from `ANALYSIS_SHUTDOWN_GRACE_SECONDS`, and rejects zero, negative, NaN, and infinity. Five seconds is the explicitly approved bounded operational grace; it is neither public API nor a reinterpretation of `request_deadline_seconds`.
It was chosen to bound shutdown while giving a nearly completed active invocation time to commit its
actual terminal outcome.

`main.py` passes this value to `AnalysisWorker`. On shutdown, admission closes and queued jobs retain the existing cancellation policy. The worker does not claim another job after shutdown starts. If one job is active, the worker waits at most the configured grace for its processor to return and record `COMPLETED` or `FAILED`; only grace expiry cancels the active task and records `CANCELLED`. Idle workers are cancelled promptly. `AnalysisQueue.mark_finished` no longer overwrites an already terminal state.
