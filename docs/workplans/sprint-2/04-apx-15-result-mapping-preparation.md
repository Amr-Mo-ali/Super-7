...
| Physical + Ball can produce Overall without Technical | **Yes.** Overall may be calculated from any two of the three core weighted categories—Technical, Physical Activity, and Ball Involvement—then renormalizes their configured weights. Physical + Ball therefore produces Overall with effective weights `0.30/0.55` and `0.25/0.55`. | [`PlayerRatingEngine._overall`](../../../src/services/player_rating/engine.py#L148), [`PlayerRatingConfig`](../../../src/services/player_rating/config.py#L24) |
| Optional Game Intelligence can cause an Overall `KeyError` | **Yes, latent.** `summarize()` adds an available Game Intelligence category, and the generic available-category gate can count it toward eligibility, but `_overall()` has weights only for Technical, Physical, and Ball and indexes the weight map by every available category. Production currently avoids the path by not passing Game Intelligence into `summarize()`; filtering is not implemented in the engine. Game Intelligence is intended to remain excluded from Overall and must be explicitly ignored or filtered by the future corrected runtime. | [`summarize`](../../../src/services/player_rating/engine.py#L35), [`_overall`](../../../src/services/player_rating/engine.py#L148), [production call](../../../src/api/routes.py#L1773) |
...
| `overall` | Overall Rating intended to use the available core weighted categories | Technical, Physical Activity, and Ball Involvement are the three configured weighted categories. The optional Game Intelligence input can also be appended to the generic available-category list even though it has no Overall weight. | `_overall()` requires at least two entries marked `available`, then looks up each entry in the configured weight map and renormalizes. For core-only inputs, any two of Technical `0.45`, Physical `0.30`, and Ball `0.25` are valid. Physical + Ball therefore uses effective weights `0.30/0.55` and `0.25/0.55`. An available Game Intelligence entry is not filtered and can raise `KeyError` during weight lookup. | Unitless `0..100`; level bands are `<20 very_low`, `<35 low`, `<50 developing`, `<65 moderate`, `<80 good`, `<90 very_good`, otherwise `excellent` | Safe eligibility is any two of the three core weighted categories; Technical is not mandatory. The current generic available-category gate does not enforce the intended Game Intelligence exclusion. | Mean included-category confidence × `min(1, evidence duration/5)` × `included category count/3`, clamped to `0..1`; evidence adequacy, not calibrated correctness | `insufficient_supported_categories` when fewer than two categories are marked available; an available unweighted category can instead reach the latent `KeyError` path | Provisional composite metric with a current category-filtering correctness defect; not validated rating |
...
- Technical is not mandatory. Overall can be calculated safely from any two of the three core weighted categories: Technical, Physical Activity, and Ball Involvement. This is not an arbitrary-any-available-category rule.
- Missing core categories are correctly excluded and the remaining core weights are renormalized; they are not replaced by zero. Physical + Ball is valid and uses effective weights `0.30/0.55` and `0.25/0.55`.
...
- Available Game Intelligence passed to the engine can be counted by the generic available-category gate, but it is structurally incompatible with the three-entry weight map and can raise `KeyError`. This is latent because the production call omits the optional parameter; the engine does not currently implement the intended filtering. Game Intelligence must remain excluded from Overall and be explicitly ignored or filtered by the future corrected runtime. This document does not implement that fix; it remains owned by scoring/correctness implementation and RED/GREEN contract work.

### Proposed canonical `overallEvidenceScore` policy

`overallEvidenceScore` is **a provisional evidence-weighted composite of the available members of the three canonical weighted categories**. It is not an overall football-ability rating and is not scientifically validated. The canonical any-two rule is limited to Technical Event Evidence, Physical Activity, and Ball Involvement; it does not admit Game Intelligence or another arbitrary available category.
...
| Exactly two | Calculate `overallEvidenceScore` from only the two available core weighted categories and renormalize their existing base weights to sum to `1.0`. | `PARTIAL` | Included categories, missing category, renormalized effective weights, evidence-confidence meaning, limitations, and formula version |
...
|---|---|---|---|---|---|---|
| `overallEvidenceScore` | Current `overall`; Apex candidate `super7Score` | `PARTIAL_MATCH`, `SEMANTIC_RENAME_REQUIRED`, `TARGET_SELECTION_GATED`, `EVIDENCE_GATED`, `SCIENTIFIC_VALIDATION_REQUIRED` | Established visual target plus at least two of the three core weighted categories | `INCLUDE_WITH_CANONICAL_NAME`; implementation and validation gated | Provisional evidence-weighted composite, never a scalar ability claim | Super-7 |
...
- `overallEvidenceScore` is a provisional evidence-weighted composite of any two or all three core weighted categories; it is not an overall football-ability rating. Game Intelligence and arbitrary available categories are excluded.
...
| Optional Game Intelligence can cause latent Overall `KeyError` | The generic available-category gate can count an available Game Intelligence result, which then reaches a missing weight-map key. No engine filtering currently implements the intended exclusion. | A future accidental admission could fail callback production even though Game Intelligence is deferred from canonical v1. | High | Super-7 scoring | RED/GREEN contract and implementation: explicitly ignore/filter Game Intelligence and other deferred or unknown categories so only the three core weighted categories can enter Overall or affect eligibility. | Yes for any future GI admission; no while excluded |
...
- `overallEvidenceScore` is included as a provisional evidence-weighted composite. Technical Event Evidence is not independently mandatory: all three core weighted categories produce `COMPLETE`, exactly any two of those three produce `PARTIAL` with renormalized base weights, and fewer than two produce `INSUFFICIENT_EVIDENCE` with `value=null`. Game Intelligence remains deferred and excluded.
...
- The canonical `overallEvidenceScore` partial/complete/insufficient envelope, with explicit filtering so Game Intelligence and other deferred or unknown categories cannot enter Overall or affect eligibility.
