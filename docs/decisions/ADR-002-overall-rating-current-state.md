...
`overall` is numeric only when at least two `available` categories exist. The only weighted categories are `technical_skill` (0.45), `physical_activity` (0.30), and `ball_involvement` (0.25). Available weights are renormalized to sum to 1; missing categories are not zero-filled. `game_intelligence` has no overall weight and does not enter current production overall.
...
| Game intelligence available | It remains excluded; its availability does not satisfy the two-category requirement. |
